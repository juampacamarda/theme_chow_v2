...
// Función para manejar errores de EXIF silenciosamente
function chow_handle_exif_error($errno, $errstr, $errfile, $errline) {
    // Solo suprimir warnings relacionados con EXIF
    if (strpos($errstr, 'exif_read_data') !== false || strpos($errstr, 'EXIF') !== false) {
        return true; // Suprimir el error
    }
    return false; // Dejar que otros errores se muestren
}
set_error_handler('chow_handle_exif_error', E_WARNING);

// Filtro adicional para manejar errores en wp_generate_attachment_metadata
